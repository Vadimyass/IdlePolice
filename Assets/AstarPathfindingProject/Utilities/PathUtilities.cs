...
namespace Pathfinding {
	...
	public static class PathUtilities {
		...
		public static List<Vector3> SimplifyPath(List<Vector3> originalPath)
		{
			if (originalPath == null || originalPath.Count < 3)
				return originalPath;

			List<Vector3> simplifiedPath = new List<Vector3> { originalPath[0] };

			for (int i = 1; i < originalPath.Count - 1; i++)
			{
				Vector3 directionToNext = (originalPath[i + 1] - originalPath[i]).normalized;
				Vector3 directionToPrevious = (originalPath[i] - originalPath[i - 1]).normalized;

				// Сравниваем направления. Если они совпадают, точка i не добавляется.
				if (Vector3.Dot(directionToNext, directionToPrevious) < 0.99f)
				{
		...
		public static List<Vector3> AddDetailToPath(List<Vector3> originalPath, int levels)
		{
			if (originalPath == null || originalPath.Count < 2 || levels < 1)
				return originalPath;

			List<Vector3> detailedPath = new List<Vector3>(originalPath);

			for (int level = 0; level < levels; level++)
			{
				List<Vector3> newPath = new List<Vector3>();

				// Проходим по всем точкам текущего пути
				for (int i = 0; i < detailedPath.Count - 1; i++)
				{
					Vector3 start = detailedPath[i];
					Vector3 end = detailedPath[i + 1];

					// Добавляем начальную точку сегмента
					newPath.Add(start);

					// Находим среднюю точку между start и end и добавляем её
				...
				newPath.Add(detailedPath[detailedPath.Count - 1]);

				// Обновляем detailedPath для следующего уровня детализации
				detailedPath = newPath;
			}
